...
<template>
  <UDashboardPage>
    <UDashboardPanel grow>
      <UDashboardNavbar class="gmsPurpleHeader" title="Customer Messages">
      </UDashboardNavbar>
      ...
      <UDashboardToolbar class="bg-gms-gray-100">
        <template #right>
          <UButton color="green" variant="outline" label="New message" trailing-icon="i-heroicons-plus"
            @click="onCreate()" />
        </template>
      </UDashboardToolbar>
      <UTable :rows="gridMeta.messages" :columns="columns" :loading="gridMeta.isLoading" class="w-full" :ui="{
  ...
  <!-- New Modal -->
  <UDashboardModal v-model="modalMeta.isCustomerModalOpen" :title="modalMeta.modalTitle"
    ...
    <CustomersMessageForm @close="handleModalClose" @save="handleModalSave"
      :selected-customer="gridMeta.selectedMessageId" :is-modal="true" />
  </UDashboardModal>
</template>
<style scoped></style>