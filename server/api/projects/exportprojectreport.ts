...
} from "~/server/controller/projects";
import { getEmployeeSchedules } from "~/server/controller/jobs";

export default eventHandler(async (event) => {
  try {
    const method = event._method;
    const {id, number} = getQuery(event)

    switch (method) {
      case "GET":
    
        const { reportData: data, job } = await getProjectReportData(id,number)
        ...
        const employeeData = await getEmployeeSchedules(null,null, {
          JobID: job.UniqueID,
          OperationID: reportData[0][0].JobOperationId,
        })
        ...
        if(reportData.length !== 0){
          ...
          <body style="font-family: Arial; max-width: 1024px; margin: 0 auto;">
            <div style="display: flex; justify-content: space-between;">
                <div>
                    <h3 style="margin: 0;">Job # ${job.NUMBER}</h3>
                </div>
                <div style="width: 20%; text-align: center;">
                    <h3 style="margin: 0;">Qty ${job.QUANTITY}</h3>
                </div>
                <div style="width: 50%; text-align: right;">
                  <h3 style="margin: 0;">${job.PART}</h3>
                </div>
            </div>`;
