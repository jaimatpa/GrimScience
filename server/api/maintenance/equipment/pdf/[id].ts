import { format } from 'date-fns';
import puppeteer from "puppeteer";
import { getPDFDataById } from "~/server/controller/maintenance/equipment";

// const base64Image = 'iVBORw0KGgoAAAANSUhEUgAAASwAAAC3CAYAAACoq2NBAAA2ZUlEQVR42uxdCZwdRZmvqu53zZFMJplMQkJICAkQIQguGhERV0WURVRAFgjkeBOOxCMzgwcugsLqIjIzrMqZeZNwCK6y6i4/UZFLQBFZhEjIuR7k'; // ... (truncated for brevity)
const base64Image = ''

function generateReportTable(reports) {
  const tableRows = reports.map((report, index) => `
    <tr>
      <td><span style="font-size: 12px;">${index + 1}</span></td>
      <td><span style="font-size: 12px;">${report.Date ? format(new Date(report.Date), 'MM/dd/yyyy') : ''}</span></td>
      <td><span style="font-size: 12px;">${report.By || ''}</span></td>
      <td><span style="font-size: 12px;">${report['Work Done'] || ''}</span></td>
      <td><span style="font-size: 12px;">${report.Parts || ''}</span></td>
    </tr>
  `).join('');

  return `
    <table style="margin-top: 40px; width: 100%;">
      <h4 style="padding-top: 30px;">Report:<br/></h4>
      <thead>
        <tr>
          <th>#</th>
          <th>Date</th>
          <th>By</th>
          <th>Work Done</th>
          <th>Parts</th>
        </tr>
      </thead>
      <tbody>
        ${tableRows}
      </tbody>
    </table>
  `;
}

export default eventHandler(async (event) => {
  try {
    const method = event._method;
    const id = event.context.params.id;

    switch (method) {
      case "GET":
        // Fetch job operations by ID
        const jobOperations = await getPDFDataById({ uniqID: id });

        console.log("okk", jobOperations.reports);
        if (!jobOperations || jobOperations.length === 0) {
          setResponseStatus(event, 404);
          return { error: "No data found for the given ID." };
        }

        // Generate HTML content
        let htmlContent = `
        <body style="font-family: Arial; max-width: 1024px; margin: 0 auto;">
          <div style="display: flex; justify-content: space-between;">
            <div style="display: flex; justify-content: space-between; align-items: center;">
              <img style="width: 105px; height: 50px;" src="data:image/png;base64,${base64Image}" alt="Grimm Avatar" />
              <h3 style="margin: 0; padding-left: 5px; padding-top: 5px; font-size: 20px;">Maintenance Record</h3>
            </div>

            <div style="width: 30%;">
              <h4 style="margin: 0;">
                <strong>Serial:</strong> <span style="font-size: 12px;">${jobOperations[0]["SERIAL"]}</span>
              </h4>
              <h4 style="margin: 0;">
                <strong>Location:</strong> <span style="font-size: 12px;">${jobOperations[0]["DATE"] ? format(new Date(jobOperations[0]["DATE"]), 'MM/dd/yyyy') : ''}</span>
              </h4>
            </div>
          </div>

          <div style="display: flex; justify-content: space-between;">
            <div>
              <h4 style="margin: 0;">
                <strong>ID No:</strong> <span style="font-size: 12px;">${jobOperations[0]["MANO"]}</span>
              </h4>
              <h4 style="margin: 0;">
                <strong>By:</strong> <span style="font-size: 12px;">${jobOperations[0]["ORDEREDBY"]}</span>
              </h4>
              <h4 style="margin: 0;">
                <strong>Date:</strong> <span style="font-size: 12px;">${jobOperations[0]["DATE"] ? format(new Date(jobOperations[0]["DATE"]), 'MM/dd/yyyy') : ''}</span>
              </h4>
            </div>

            <div style="width: 30%;">
              <h4 style="margin: 0;">
                <strong>Type:</strong> <span style="font-size: 14px;">${jobOperations[0]["TYPE"]}</span>
              </h4>
              <h4 style="margin: 0;">
                <strong>Require:</strong> <span style="font-size: 14px;">${jobOperations[0]["REQUIRED"]}</span>
              </h4>
            </div>
          </div>

          <!-- Iterate through operations -->
          ${jobOperations.map((operation) => `
          <div style="display: flex; justify-content: space-between; margin-top: 40px; border-top: 2px solid #000;">
            <div style="width: 50%; margin-top: 40px;">
              <p style="margin-bottom: 0;"><strong>Equipment:</strong><br/>
                <span style="font-size: 12px;">${operation["PART"]}</span>
              </p>
            </div>
            <div style="width: 50%; margin-top: 40px;">
              <p style="margin-top: 12px; margin-bottom: 0;">Months Between :<br/>
                <span style="font-size: 12px;">${operation["MAINTAINANCE"]}</span>
              </p>
            </div>
          </div>
          <div style="margin-top: 40px;">
            <h4 style="margin: 0;">Mfg.Name:<br/></h4>
            <span style="font-size: 12px;">${operation["MfgName"]}</span>
          </div>

          <div style="margin-top: 40px;">
            <h4 style="margin: 0;">Mfg.Model:<br/></h4>
            <span style="font-size: 12px;">${operation["mfgModel"]}</span>
          </div>

          <div style="margin-top: 40px;">
            <h4 style="margin: 0;">Maintenance Require:<br/></h4>
            <span style="font-size: 12px;">${operation["MAINTAINANCE"]}</span>
          </div>

          ${generateReportTable(operation.reports)}
          `).join('')}
        </body>
        `;

        // Launch Puppeteer with a custom timeout
        const browser = await puppeteer.launch({
          headless: true,
          timeout: 30000,
          args: ['--no-sandbox', '--disable-setuid-sandbox'],
        });

        const page = await browser.newPage();
        const pdfOptions = {
          path: "Operation.pdf",
          format: "letter",
          margin: {
            top: "40px",
            bottom: "40px",
            left: "40px",
            right: "40px",
          },
        };

        await page.setContent(htmlContent);
        const pdfBuffer = await page.pdf(pdfOptions);
        await browser.close();

        // Set response headers for PDF file download
        setResponseHeader(event, "Content-Type", "application/pdf");
        setResponseHeader(event, "Content-Disposition", "attachment; filename=Operation.pdf");
        
        return pdfBuffer;
      default:
        setResponseStatus(event, 405); 
        return { error: "Method not allowed." };
    }
  } catch (error) {
    console.error(error);
    setResponseStatus(event, 500);
    return { error: "An error occurred while generating the PDF." };
  }
});



// import { format } from "date-fns";
// import puppeteer from "puppeteer";


// import { getPDFDataById } from "~/server/controller/maintenance/equipment";


// const base64Image = ''



// export default eventHandler(async (event) => {
//   try {
//     const method = event._method;
//     const id = event.context.params.id;

//     switch (method) {
//       case "GET":
//         // Fetch job operations by ID
//         const jobOperations = await getPDFDataById({ uniqID: id });

//         console.log("okk", jobOperations.reports);
//         if (!jobOperations || jobOperations.length === 0) {
//           setResponseStatus(event, 404);
//           return { error: "No data found for the given ID." };
//         }

//         // Generate HTML content
//         let htmlContent = `
//         <body style="font-family: Arial; max-width: 1024px; margin: 0 auto;">
//           <div style="display: flex; justify-content: space-between;">
//             <div style="display: flex; justify-content: space-between; align-items: center;">
//               <img style="width: 105px; height: 50px;" src="data:image/png;base64,${base64Image}" alt="Grimm Avatar" />
//               <h3 style="margin: 0; padding-left: 5px; padding-top: 5px; font-size: 20px;">Maintenance Record</h3>
        
//             </div>



//             <div style="width: 30%;">
//             <h4 style="margin: 0;">
//                   <strong>Serial:</strong> <span style="font-size: 12px;">${jobOperations[0]["SERIAL"]}</span>
//                 </h4>
//               <h4 style="margin: 0;">
//                 <strong>Location:</strong> <span style="font-size: 12px;">${jobOperations[0]["DATE"] ? format(jobOperations[0]["DATE"], 'MM/dd/yyyy') : ''}</span>
//               </h4>
//             </div>
//           </div>



//           <div style="display: flex; justify-content: space-between;">
//             <div>
//               <h4 style="margin: 0;">
//                 <strong>ID No:</strong> <span style="font-size: 12px;">${jobOperations[0]["MANO"]}</span>
//               </h4>
//               <h4 style="margin: 0;">
//                 <strong>By:</strong> <span style="font-size: 12px;">${jobOperations[0]["ORDEREDBY"]}</span>
//               </h4>
//               <h4 style="margin: 0;">
//               <strong>Date:</strong> <span style="font-size: 12px;">${jobOperations[0]["DATE"] ? format(jobOperations[0]["DATE"], 'MM/dd/yyyy') : ''}</span>
//               </h4>
//             </div>

//             <div style="width: 30%;">
//               <h4 style="margin: 0;">
//                 <strong>Type:</strong> <span style="font-size: 14px;">${jobOperations[0]["TYPE"]}</span>
//               </h4>
//               <h4 style="margin: 0;">
//                 <strong>Require:</strong> <span style="font-size: 14px;">${jobOperations[0]["REQUIRED"]}</span>
//               </h4>
//             </div>
//           </div>

//           <!-- Iterate through operations -->
//           ${jobOperations.map((operation) => `
//           <div style="display: flex; justify-content: space-between; margin-top: 40px; border-top: 2px solid #000;">
//             <div style="width: 50%; margin-top: 40px;">
//               <p style="margin-bottom: 0;"><strong>Equipment:</strong><br/>
//                 <span style="font-size: 12px;">${operation["PART"]}</span>
//               </p>
//             </div>
//             <div style="width: 50%; margin-top: 40px;">
//               <p style="margin-top: 12px; margin-bottom: 0;">Months Between :<br/>
//                 <span style="font-size: 12px;">${operation["MAINTAINANCE"]}</span>
//               </p>
//             </div>
//           </div>
//           <div style="margin-top: 40px;">
//             <h4 style="margin: 0;">Mfg.Name:<br/></h4>
//             <span style="font-size: 12px;">${operation["MfgName"]}</span>
//           </div>

//           <div style="margin-top: 40px;">
//             <h4 style="margin: 0;">Mfg.Model:<br/></h4>
//             <span style="font-size: 12px;">${operation["mfgModel"]}</span>
//           </div>

//           <div style="margin-top: 40px;">
//             <h4 style="margin: 0;">Maintenance Require:<br/></h4>
//             <span style="font-size: 12px;">${operation["MAINTAINANCE"]}</span>
//           </div>

//           <table style="margin-top: 40px; width: 100%;">
//           <h4 style="padding-top: 30px;">Report:<br/></h4>
//             <thead>
//               <tr>
//                 <th>#</th>
//                 <th>Date</th>
//                 <th>By</th>
//                 <th>Work Done</th>
//                 <th>Parts</th>
//               </tr>
//             </thead>
            
//             <tbody>
//               <tr>
//                <td><span style="font-size: 12px;">${operation["Parts"]}</span></td>
//                 <td><span style="font-size: 12px;">${jobOperations[0]["DATE"] ? format(jobOperations[0]["DATE"], 'MM/dd/yyyy') : ''}</td>
//                 <td><span style="font-size: 12px;">${operation["PART"]}</span></td>
//                 <td><span style="font-size: 12px;">${operation["Work Done"]}</span></td>
//                 <td><span style="font-size: 12px;">${operation["Parts"]}</span></td>
//               </tr>
//               <tr>
//                 <td><span style="font-size: 12px;">${operation["Parts"]}</span></td>
//                 <td><span style="font-size: 12px;">${operation["CATAGORY"]}</span></td>
//                 <td><span style="font-size: 12px;">${operation["ENGAPPCATAGORYROVAL"]}</span></td>
//                 <td><span style="font-size: 12px;">${operation["CATAGORY"]}</span></td>
//               </tr>
//               <tr>
//                <td><span style="font-size: 12px;">${operation["Parts"]}</span></td>
//                 <td><span style="font-size: 12px;">${operation["CATAGORY"]}</span></td>
//                 <td><span style="font-size: 12px;">${operation["CATAGORY"]}</span></td>
//                 <td><span style="font-size: 12px;">${operation["MARCOCATAGORYMMENTS"]}</span></td>
//               </tr>
//               <tr>
//              <td><span style="font-size: 12px;">${operation["Parts"]}</span></td>
//                 <td><span style="font-size: 12px;">${operation["CATAGORY"]}</span></td>
//                 <td><span style="font-size: 12px;">${operation["CATAGORY"]}</span></td>
//                 <td><span style="font-size: 12px;">${operation["CATAGORY"]}</span></td>
//               </tr>
//             </tbody>
//           </table>
//           `).join('')}
//         </body>
//         `;

//         // Launch Puppeteer with a custom timeout
//         const browser = await puppeteer.launch({
//           headless: true,
//           timeout: 30000, // Timeout for browser launch
//           args: ['--no-sandbox', '--disable-setuid-sandbox'], // For optimization
//         });

//         const page = await browser.newPage();
//         const pdfOptions = {
//           path: "Operation.pdf",
//           format: "letter",
//           margin: {
//             top: "40px",
//             bottom: "40px",
//             left: "40px",
//             right: "40px",
//           },
//         };

//         await page.setContent(htmlContent);
//         const pdfBuffer = await page.pdf(pdfOptions);
//         await browser.close();

//         // Set response headers for PDF file download
//         setResponseHeader(event, "Content-Type", "application/pdf");
//         setResponseHeader(event, "Content-Disposition", "attachment; filename=Operation.pdf");
        
//         return pdfBuffer;
//       default:
//         setResponseStatus(event, 405); 
//         return { error: "Method not allowed." };
//     }
//   } catch (error) {
//     console.error(error);
//     setResponseStatus(event, 500); // Internal Server Error
//     return { error: "An error occurred while generating the PDF." };
//   }
// });


// export default eventHandler(async (event) => {
//   try {
//     const method = event._method;
//     const id = event.context.params.id;

//     switch (method) {
//       case "GET":
//         const jobOperations = await getJobOperationsById({ uniqID: id });
//         if (!jobOperations || jobOperations.length === 0) {
//           setResponseStatus(event, 404);
//           return { error: "No data found for the given ID." };
//         }
//         // Generate HTML content
//         let htmlContent = `
//         <body style="font-family: Arial; max-width: 1024px; margin: 0 auto;">


//           <div style="display: flex; justify-content: space-between;">
//                 <div style="display: flex; justify-content: space-between; align-items: center;">
//                 <img style="width:105px; height:50px;" src="data:image/png;base64,${base64Image}" alt="Grimm Avatar" />
//                 <h3 style="margin: 0; padding-left: 5px;padding-top: 5px; font-size:20px;">Maintenance Record</h3>

//           <div style="margin-left: 20px;">
//       <h4 style="margin: 0;">
//         <strong>Serial:</strong> <span style="font-size: 12px;">${jobOperations[0]["uniqueID"]}</span>
//       </h4>
//     </div>



//             </div>


             
               
                
      
            
//             <div style="width: 30%;">
//               <h4 style="margin: 0;">
//                 <strong>Serial :</strong> <span style="font-size: 12px;">${jobOperations[0]["uniqueID"]}</span>
//               </h4>

//              <h4 style="margin: 0;">
//               <strong>Locaton</strong> <span style="font-size: 12px;">${jobOperations[0]["ENGDATEAPPROVED"] ? format(jobOperations[0]["ORIGINATORDATE"], 'MM/dd/yyyy') : ''}</span>
//               </h4>
//             </div>


         
//           </div>
          





          
//             <div style="display: flex; justify-content: space-between;">
//                 <div>
//         <h4 style="margin: 0;">
//         <strong>ID No:</strong> <span style="font-size: 12px;">${jobOperations[0]["uniqueID"]}</span>
//       </h4>
//    <h4 style="margin: 0;">
//         <strong>By:</strong> <span style="font-size: 12px;">${jobOperations[0]["uniqueID"]}</span>
//       </h4>
//         <h4 style="margin: 0;">
//         <strong>Date:</strong> <span style="font-size: 12px;">${jobOperations[0]["uniqueID"]}</span>
//       </h4>
//             </div>


        
//             <div style="width: 30%;">
              
//             <h4 style="margin: 0;">
//               <strong>Type</strong> <span style="font-size: 14px;">${jobOperations[0]["ENGDATEAPPROVED"] ? format(jobOperations[0]["ORIGINATORDATE"], 'MM/dd/yyyy') : ''}</span>
//               </h4>

//               <h4 style="margin: 0;">
//               <strong>Requirt</strong> <span style="font-size: 14px;">${jobOperations[0]["ENGDATEAPPROVED"] ? format(jobOperations[0]["ORIGINATORDATE"], 'MM/dd/yyyy') : ''}</span>
//               </h4>

          
//             </div>


         
//           </div>
          
//           `;

//         // Iterate through operations
//         jobOperations.forEach((operation, index) => {
//           htmlContent += `
       
//           <div style="display: flex; justify-content: space-between; margin-top:40px; border-top: 2px solid #000;">
//             <div style="width: 50%; margin-top:40px;">
//               <p style=" margin-bottom: 0;"> <strong>Description:</strong>  <br/>
              
//               <span style="font-size: 12px;">${jobOperations[0]["DESCRIPTION"]}</span>
              
//             </div>
//             <div style="width: 50%; margin-top:40px;">
//               <p style="margin-top: 12px; margin-bottom: 0;">Reason for Change: <br/>
              
//               <span style="font-size: 12px;">${jobOperations[0]["REASONFORCHANGE"]}</span>
          
//             </div>
//           </div>


//           <div style="display: flex; justify-content: space-between; margin-top: 40px;">
//             <div style="width: 50%;">
//               <p style="margin-top: 12px; margin-bottom: 0;">Issue:<br/>
//                  <span style="font-size: 12px;">${jobOperations[0]["ISSUE"]}</span>
            
//             </div>
//             <div style="width: 50%;">
//               <p style="margin-top: 12px; margin-bottom: 0;">Solution:<br/>
//                  <span style="font-size: 12px;">${jobOperations[0]["SOLUTION"]}</span>
//             </div>
//           </div>



//           <div style="margin-top: 40px;">
//             <h4 style="margin: 0;"> Parts Affected:<br/></h4>
//              <span style="font-size: 12px;">${jobOperations[0]["PARTS"]}</span>
         
//             </div>

//                <div style="margin-top: 40px;">
//             <h4 style="margin: 0;"> Product Affected:<br/></h4>
//              <span style="font-size: 12px;">${jobOperations[0]["ProductsDetails"]}</span>
         
//             </div>

//           <div style="margin-top: 40px;">
//             <h4 style="margin: 0;"> EQUIPMENT Affected:<br/></h4>
//              <span style="font-size: 12px;">${jobOperations[0]["EQUIPMENT"]}</span>
         
//             </div>



//         <table style="margin-top: 40px; width: 100%;">
//     <thead>
//         <tr>
//             <th></th>
//             <th>Employee</th>
//             <th>Date</th>
//             <th>Approval</th>
//             <th>Comment</th>
//         </tr>
//     </thead>
//     <tbody>
//         <tr>
//             <td><span>Originator</span></td>
//             <td><span style="font-size: 12px;">${jobOperations[0]["ORIGINATOR"]}</span></td>
//             <td><span style="font-size: 12px;">${jobOperations[0]["ORIGINATORDATE"]}</span></td>
//             <td><span style="font-size: 12px;">${jobOperations[0]["ENGAPPROVAL"] }</span></td>
//             <td><span style="font-size: 12px;">${jobOperations[0]["ENGCOMMENTS"] }</span></td>
//         </tr>
//         <tr>
//             <td><span>Engineering</span></td>
//             <td><span style="font-size: 12px;">${jobOperations[0]["ENGAPPROVER"]}</span></td>
//             <td><span style="font-size: 12px;">${jobOperations[0]["ENGDATEAPPROVED"] ? format(jobOperations[0]["ENGDATEAPPROVED"], 'MM/dd/yyyy') : ''}</span></td>
//             <td><span style="font-size: 12px;">${jobOperations[0]["ENGAPPROVAL"] }</span></td>
//             <td><span style="font-size: 12px;">${jobOperations[0]["ENGCOMMENTS"] }</span></td>
//         </tr>
//         <tr>
//             <td><span>Marketing</span></td>
//             <td><span style="font-size: 12px;">${jobOperations[0]["MARAPPROVER"]}</span></td>
//             <td><span style="font-size: 12px;">${jobOperations[0]["MARDATEAPPROVED"] ? format(jobOperations[0]["MARDATEAPPROVED"], 'MM/dd/yyyy') : ''}</span></td>
//             <td><span style="font-size: 12px;">${jobOperations[0]["MARAPPROVAL"] }</span></td>
//             <td><span style="font-size: 12px;">${jobOperations[0]["MARCOMMENTS"] }</span></td>
//         </tr>
//         <tr>
//             <td><span>Manufacturing</span></td>
//             <td><span style="font-size: 12px;">${jobOperations[0]["MANAPPROVER"]}</span></td>
//             <td><span style="font-size: 12px;">${jobOperations[0]["MANDATEAPPROVED"] ? format(jobOperations[0]["MANDATEAPPROVED"], 'MM/dd/yyyy') : ''}</span></td>
//             <td><span style="font-size: 12px;">${jobOperations[0]["MANAPPROVAL"] }</span></td>
//             <td><span style="font-size: 12px;">${jobOperations[0]["MANCOMMENTS"] }</span></td>
//         </tr>
//     </tbody>
// </table>
//             </div>`;
//         });

//         // Launch Puppeteer with a custom timeout
//         const browser = await puppeteer.launch({
//           headless: true,
//           timeout: 30000, // Timeout for browser launch
//           args: ['--no-sandbox', '--disable-setuid-sandbox'], // For optimization
//         });

//         const page = await browser.newPage();
//         const pdfOptions = {
//           path: "Operation.pdf",
//           format: "letter",
//           margin: {
//             top: "40px",
//             bottom: "40px",
//             left: "40px",
//             right: "40px",
//           },
//         };

//         // Set content with a 30s timeout
//         await page.setContent(htmlContent, { waitUntil: "networkidle2", timeout: 30000 });
//         const pdfBuffer = await page.pdf(pdfOptions);
//         await browser.close();

//         // Set headers and return the PDF buffer
//         setHeaders(event, {
//           "Content-Type": "application/pdf",
//           "Content-Disposition": 'inline; filename="View Operation.pdf"',
//           "Page-Size": "Letter",
//         });
//         return pdfBuffer;

//       default:
//         setResponseStatus(event, 405);
//         return { error: "Method Not Allowed" };
//     }
//   } catch (error) {
//     console.error("Error generating PDF:", error);
//     throw new Error(`Error fetching data from table: ${error.message}`);
//   }
// });
