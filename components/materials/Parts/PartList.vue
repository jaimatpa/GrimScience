...
const emit = defineEmits(["close", "save", "refresh","select", "productFormData"]);
...
const editInit = async () => {
  ...
  selectedInstanceID.value = props.selectedPartInstace;
  selectedModal.value = props.selectedPartModel;
  getPartsData();
  loadingOverlay.value = true;
  propertiesInit();
  loadingOverlay.value = false;
...
const propertiesInit = async () => {
  ...
  if (selectedModal.value !== null) {
    getInventoryAndRequired();
  }
...
const getInventoryAndRequired = async () => {
  await useApiFetch(
    `/api/materials/parts/parts/transactions?model=${selectedModal.value}`,
    {
      method: "GET",
      onResponse({ response }) {
        if (response.status === 200) {
  ...
  await useApiFetch(
    `/api/materials/parts/totalRequired?model=${selectedModal.value}`,
    {
      method: "GET",
      onResponse({ response }) {
        if (response.status === 200) {
...
const getPoAndUsedOn = async () => {
  await useApiFetch(
    `/api/materials/parts/parts/podetails?instanceId=${selectedInstanceID.value}`,
    {
      method: "GET",
      onResponse({ response }) {
        if (response.status === 200) {
...
const getRevisions = async () => {
  if (selectedInstanceID.value) {
    await useApiFetch(
      `/api/materials/parts/revisions?instanceId=${selectedInstanceID.value}`,
      {
        method: "GET",
        onResponse({ response }) {
          if (response.status === 200) {
...
const revision = async (event: FormSubmitEvent<any>) => {
  ...
  if (selectedInstanceID.value != null) {
    await useApiFetch(
      `/api/materials/parts/parts/revision?instanceIdForRevision=${selectedInstanceID.value}&id=${selectedPartsID.value}`,
      {
        method: "PUT",
        body: event.data,
        onResponse({ response }) {
...
const onSelect = async (row) => {
  ...
  gridMeta.value.description = row?.DESCRIPTION;
};
