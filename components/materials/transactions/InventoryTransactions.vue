...
const onInventoryDetailSelect = async (row) => {
    if (JSON.stringify({ ...inventoryDetailGridMeta.value.selectedDetail, class: "" }) === JSON.stringify({ ...row, class: "" }))
...
<template>
    ...
    <UForm :validate="validate" :validate-on="['submit']" :state="formData" @submit="onSubmit">
        <div class="flex flex-col">
            <div class="flex justify-between bg-slate-400 px-3 py-1">
                <div class="flex items-center">Inventory transactions</div>
            ...
            <div class="py-2">
                            ...
                            <template v-if="column.kind !== 'actions'">
                                <div class="px-1 py-1">
                                    <CommonSortAndInputFilter
                                        ...
                                        :sort-key="column.key"
                                        :sort-icon="
                                            column?.sortDirection === 'none' ? noneIcon : column?.sortDirection === 'asc' ? ascIcon : descIcon
                                        "
                ...
                <div class="flex items-center bg-slate-400 px-3 py-1 mt-2">Parts Used</div>
                ...
                <div class="flex flex-row">
                    ...
                    <div class="basis-1/2 p-4 pt-0 flex flex-row space-x-3">
                        <div class="basis-1/4 flex justify-center">Transaction#: {{ formData.transactionID }}</div>
                            ...
                            <UFormGroup label="Date">
                                <UPopover :popper="{ placement: 'bottom-start' }">
                                    <UButton
                                        ...
                                        :label="formData.Dated && format(formData.Dated, 'MM/dd/yyyy')"
                                        variant="outline"
        ...
        <div class="flex justify-between gap-3 px-8">
            <div>
                <UButton
                    ...
                    color="red"
                    variant="outline"
                    :ui="{ base: 'min-w-[200px]', truncate: 'flex justify-center w-full' }"
                    truncate
                    @click="onDelete" />
            </div>
            <div class="flex flex-row space-x-5">
    ...
    <UDashboardModal
        v-model="isServiceReportModalOpen"
        ...
        <ServiceReportDetail :selected-complaint="null" :selected-service-report="selectedServiceReportID" @save="editInit" @close="" />
    </UDashboardModal>
    ...
    <UDashboardModal
        v-model="isVendorInvoiceModalOpen"
        title="Order"
        :ui="{
            title: 'text-lg',
            header: {
