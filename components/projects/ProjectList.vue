...
const handleDeleteClick = async () => {
  if(selected.value.length > 0){
    ...
    await useApiFetch(`/api/projects/deleteProjects`, {
      method: "DELETE",
...
const onDblClick = async () => {
  if (gridMeta.value.selectedProjectId) {
    modalMeta.value.modalTitle = "Project Information";
    modalMeta.value.isProjectFormModalOpen = true;
  }
};

const onCreate = () => {
  gridMeta.value.selectedProjectId = null;
  modalMeta.value.modalTitle = "New Project";
  modalMeta.value.isProjectFormModalOpen = true;
};

const onUpdatePercentage = async () => {
  const jobs = gridMeta.value.projects.map(item => {
    return item.UniqueID
  })
  loadingOverlay.value = true
  await useApiFetch("/api/projects/updateprojectpercentage", {
    method: "PUT",
    body: { jobs }
  });
...
const openProjectDetailsForm = (projectId) => {
  gridMeta.value.selectedProjectId = projectId
}
</script>
...
<template>
  ...
  <UDashboardPage>
      ...
      <UDashboardToolbar>
        ...
        <template #right>
          ...
          <UButton
            ...
            label="Update Project Percentages"
            variant="outline"
            color="blue"
    ...
    <!-- Job Detail Modal -->
  <UDashboardModal 
    v-model="modalMeta.isProjectFormModalOpen" 
  ...
  >
    <ProjectsProjectForm
      ...
      :selected-job="gridMeta.selectedProjectId"
      @open="openProjectDetailsForm"
