...
const categories = ref(["Marketing", "Accounting", "Engineering", "Manufacturing"]);
...
const editInit = async () => {
  ...
  await useApiFetch(`/api/projects/${props.selectedJob}`, {
    method: "GET",
    onResponse({ response }) {
...
const validate = (state: any): FormError[] => {
  const errors = [];
  if (!state.NUMBER) errors.push({ path: 'NUMBER', message: 'Please enter your Project#.' })
  ...
  if (!state.Catagory) errors.push({ path: 'Catagory', message: 'Please enter a category .' })
  if (!state.SubCatagory) errors.push({ path: 'SubCatagory', message: 'Please enter a sub category.' })
  if (!state.PART) errors.push({ path: 'PART', message: 'Please enter a part.' })
  return errors;
};
...
const handleRefreshProject = async () => {
  ...
  loadingOverlay.value = true
  await useApiFetch(`/api/projects/refreshProject/`, {
    method: "GET",
    params: { jobId: props.selectedJob, qty:  formData.QUANTITY },
    onResponse({ response }) {
      if (response.status === 200) {
...
const handleViewOperationClick = () => {
  window.open(`/api/projects/exportprojectreport/?id=${formData.InstanceID}&number=${formData.NUMBER}`);
};
...
const handlePutIntoInventory = async () => {
  if(sbQty.value){
    if(formData.PerType === 'Serial/Unit'){
      loadingOverlay.value = true
      await useApiFetch(`/api/projects/putintoinventory/`, {
        method: "PUT",
...
<template>
  ...
  <UForm :validate="validate" :validate-on="['submit']" :state="formData" class="space-y-4 p-3" @submit="onSubmit">
    <div class="flex space-x-4">
      <div>
        <div class="flex flex-col space-y-4">
          <div class="flex flex-row space-x-3">
            <div class="basis-1/4">
              <UFormGroup label="Project#" name="NUMBER">
            ...
            <div class="basis-1/4">
              <UFormGroup label="Project Type" name="JobType">
                <USelect v-model="formData.JobType" :options="jobTypes" />
              </UFormGroup>
            </div>
            <div class="basis-1/4">
              <UFormGroup label="Latest Cost ($)" name="Latest Cost">
                <UInput 
          ...
          <div class="flex flex-row space-x-4">
            ...
            <div class="basis-1/4">
              <UFormGroup label="Project Closed" name="Job Closed">
                <UPopover :popper="{ placement: 'bottom-start' }">
      ...
      <div class="flex flex-col gap-4 justify-start ">
        ...
        <div class="">
          <UButton icon="i-heroicons-magnifying-glass" variant="outline" color="green" label="View Instructions"
    ...
    <div v-if="formData.JobType === 'Sub Assembly' || formData.JobType === null">
      <UTabs :items="tabitems" class="">
        <template #product="{ item }">
          ...
          <div class="flex flex-col space-y-3">
            ...
            <div class="w-1/4">
              <UFormGroup label="Sub Category" name="SubCatagory">
  ...
  >
    <ProjectsProjectListModal @close="handleJobSelect" />
  </UDashboardModal>
