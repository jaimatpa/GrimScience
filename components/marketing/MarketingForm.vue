...
<template>
    ...
    <UForm :validate="validate" :validate-on="['submit']" :state="formData" class="space-y-4" @submit="onSubmit">
      ...
      <div class="flex flex-row space-x-3">
        ...
        <div class="basis-1/2">
          <UFormGroup label="Ready To Produce" name="RTOProduce">
            <UPopover :popper="{ placement: 'bottom-start' }">
              <UButton icon="i-heroicons-calendar-days-20-solid" :label="format(form.ProductionDate, 'd MMM, yyy')"
                variant="outline" :ui="{ base: 'w-full', truncate: 'flex justify-center w-full' }" truncate />
              <template #panel="{ close }">
        ...
        <div class="basis-1/2">
          <UFormGroup label="By" name="By">
            <UInputMenu v-model="form.ProductionBy" :options="employeeOptions" />
          </UFormGroup>
        </div>
      </div>
      <div class="flex flex-row space-x-3">
        <div class="basis-1/2">
          <UFormGroup label="Project Closed" name="PClosed">
            <UPopover :popper="{ placement: 'bottom-start' }">
              <UButton icon="i-heroicons-calendar-days-20-solid" :label="format(form.DATECLOSED, 'd MMM, yyy')"
                variant="outline" :ui="{ base: 'w-full', truncate: 'flex justify-center w-full' }" truncate />
              <template #panel="{ close }">
          ...
          <template #item="{ item }">
            ...
            <div v-if="item.key === 'sub'" class="Category">
              <div class="flex flex-row space-x-2">
                <UFormGroup label="Category" class="basis-1/2" name="Category">
                  ...
                  <UInputMenu @change="subCategories" v-model="form.Catagory" :options="category" />
                </UFormGroup>
                <UFormGroup label="Sub Category" class="basis-1/2" name="Sub Category">
                  <UInputMenu :options="subCategorielist" v-model="form.SubCatagory" @change="part" />
                </UFormGroup>
              ...
              <div class="flex flex-row space-x-2 mt-2">
                ...
                <div class="mt-6">
                  <UPopover :popper="{ placement: 'bottom-start' }">
                    <UButton icon="i-heroicons-calendar-days-20-solid"
                      :label="format(newEntry.dateEntered, 'd MMM, yyy')" variant="outline"
                ...
                <div class="basis-1/3 mt-6">
                  ...
                  <UButton color="cyan" variant="outline" icon="i-heroicons-pencil-square" type="submit"
                    label="Put into Inventory" @click="addInventory()" />
            ...
            <div v-else-if="item.key === 'Operation' && props.selectedCustomer !== null" class="space-y-3">
              <div class="grid grid-cols-1 mt-6 h-48">
                ...
                <UTable :rows="weekly" :columns="gridMeta.defaultColumns" @select="onSelect" @click="handleRowClick"
                ...
                <div class="basis-1/2 ">
                  <div class="flex flex-row mt-6 ">
                    <div class="basis-1/2">
                      ...
                      <UTooltip text="Edit">
                        <UButton color="cyan" variant="outline" icon="i-heroicons-pencil-square" type="submit"
                          label="Rework Parts" @click="onUsed()" />
                      </UTooltip>
                  ...
                  <div class="mt-5 flex justify-center">
                    <UTooltip text="Edit">
                      <UButton color="green" variant="outline" icon="i-heroicons-plus" type="submit"
              ...
              <div class="flex flex-row">
                <div class="basis-1/2">
                  ...
                  <UTooltip text="Edit">
                    <UButton class="text-[#1c96c5]" variant="outline" icon="i-heroicons-pencil-square" type="submit"
              ...
              <div class="basis-1/2 ">
                <div>
                  <UFormGroup label="Category" class="basis-1/2" name="Category">
                    ...
                    <UInputMenu @change="productItem" v-model="selectCategoryForList" :options="category" />
                  </UFormGroup>
                </div>

                <div>
                  <UFormGroup label="Project Item" class="basis-1/2 mt-6" name="ProjectItem">
                    <UInputMenu v-model="selectedProjectItem" :options="filteredData" />
                  </UFormGroup>
                ...
                <div class="basis-1/2 mt-6 ">
                  <UButton class="text-[#1c96c5] ml-3" variant="outline" icon="i-heroicons-magnifying-glass"
                    type="submit" label="Add Job" @click="addJob" />
                </div>
      ...
      <div class="flex justify-end gap-3">
        <UTooltip text="Edit">
          ...
          <UButton class="text-[#1c96c5] ml-3" variant="outline" icon="i-heroicons-magnifying-glass" type="submit"
            label="View Instructions" />

          <UButton class="text-[#1c96c5] ml-3" variant="outline" icon="i-heroicons-magnifying-glass" type="submit"
            label="View Parts Listing" @click="onPartListing()" />
        </UTooltip>
        <UButton color="cyan" variant="outline" type="" label="Save" @click="onSubmit" />
      </div>
    </UForm>
  </template>
  <!-- is Part Modal -->
  <UDashboardModal v-model="modalMeta.isPartsUsed" :ui="{
    ...
    <PartsUsed :selected-customer="selectedCustomer" @close="modalMeta.isPartsUsed = true" />
  </UDashboardModal>
  <!-- is Part Listing Modal -->
  <UDashboardModal v-model="modalMeta.isPartLisingModalOpen" :ui="{
    title: 'text-lg',
    header: { base: 'flex flex-row min-h-[0] items-center', padding: 'pt-5 sm:px-9' },
  ...
  <UDashboardModal v-model="modalMeta.manuFactureModal" :ui="{
    title: 'text-lg',
    header: { base: 'flex flex-row min-h-[0] items-center', padding: 'pt-5 sm:px-9' },
    body: { base: 'gap-y-1', padding: 'sm:pt-0 sm:px-9 sm:py-3 sm:pb-10' },
    width: 'w-[1500px] sm:max-w-9xl',
