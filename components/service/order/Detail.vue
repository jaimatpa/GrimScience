      ...
      if (response.status === 200) {
        ...
        if (employees?.length) {
          const formattedEmployees = employees.map(
            (employee) =>
              `#${employee.payrollnumber || "n/a"} ${employee.fname || ""} ${
                employee.lname || ""
              }`
          );
...
const onSerialSelect = async (row) => {
  if (
    JSON.stringify({ ...serialGridMeta.value.selectedSerial, class: "" }) !==
    ...
    serviceOrderInfo.value.SERIALNO =
      serialGridMeta.value.selectedSerial.serial;
    ...
    if (serialGridMeta.value.selectedSerial) {
      ...
      let tmpRECBYOptions = complaintGridMeta.value.complaints.map(
        (item: any) => item.RECBY
      );
      tmpRECBYOptions = tmpRECBYOptions.filter(
        (item) => item !== "" && item !== null
      );
      ...
      serviceOrderInfo.value.PRODUCTDESC =
        complaintGridMeta.value.complaints[0].PRODUCTDESC;
      serviceOrderInfo.value.RECBYOptions = filteredRECBYOptions;
      serviceOrderInfo.value.RECBYOptions.unshift(null);
      let tmpReasonOptions = complaintGridMeta.value.complaints.map(
        (item: any) => item.ValidComplaintReason
      );
      tmpReasonOptions = tmpReasonOptions.filter(
        (item) => item !== "" && item !== null
      );
...
const onComplaintSelect = async (row) => {
  ...
  if (complaintGridMeta.value.selectedComplaint) {
    complaintUniquueId.value =
      complaintGridMeta.value.selectedComplaint.uniqueID;
    serviceOrderInfo.value.SERIALNO =
      complaintGridMeta.value.selectedComplaint.SERIALNO;
    serviceOrderInfo.value.COMPLAINTNUMBER =
      complaintGridMeta.value.selectedComplaint.COMPLAINTNUMBER;
    serviceOrderInfo.value.COMPLAINTDATE =
      complaintGridMeta.value.selectedComplaint.COMPLAINTDATE;
    serviceOrderInfo.value.COMPLAINT =
      complaintGridMeta.value.selectedComplaint.COMPLAINT;
    serviceOrderInfo.value.PRODUCTDESC =
      complaintGridMeta.value.selectedComplaint.PRODUCTDESC;
    serviceOrderInfo.value.RECBY =
      complaintGridMeta.value.selectedComplaint.RECBY;
    serviceOrderInfo.value.ValidComplaintReason =
      complaintGridMeta.value.selectedComplaint.ValidComplaintReason;
    serviceOrderInfo.value.FAILINVEST =
      complaintGridMeta.value.selectedComplaint.FAILINVEST;
    serviceOrderInfo.value.OPENCASE =
      complaintGridMeta.value.selectedComplaint.OPENCASE;
    serviceOrderInfo.value.INJURYREPORTNO =
      complaintGridMeta.value.selectedComplaint.INJURYREPORTNO;
    serviceOrderInfo.value.CLOSEDOUTBY =
      complaintGridMeta.value.selectedComplaint.ClosedOutBy;
    WARRANTYUNTIL.value =
      complaintGridMeta.value.selectedComplaint.WARRANTYUNTIL;
...
const onServiceReportDblClick = () => {
  selectedServiceReportID.value =
    serviceReportGridMeta.value.selectedServiceReport?.uniqueID;
...
const onInvestigationRemoveBtnClick = async () => {
  if (investigationGridMeta.value.selectedInvestigation) {
    await useApiFetch(
      `/api/engineering/investigationcomplaints/${
        investigationGridMeta.value.selectedInvestigation?.uniqueid ?? 0
      }`,
      {
        method: "DELETE",
        onResponse({ response }) {
...
const onPreviewOrderViewBtnClick = () => {
  if (complaintGridMeta.value.selectedComplaint?.uniqueID) {
    window.open(
      `/api/service/orders/exportcomplaints/${complaintGridMeta.value.selectedComplaint?.uniqueID}`
    );
  } else {
    toast.add({
      description: "Please select order first",
...
const validate = (state: any): FormError[] => {
  const errors = [];
  if (!state.COMPLAINTDATE)
    errors.push({ path: "complaintDate", message: "Required" });
  if (state.INJURYREPORTNO === null)
    errors.push({ path: "injuryReportNo", message: "Required" });
  if (state.OPENCASE === null)
    errors.push({ path: "openCase", message: "Required" });
...
watch(
  () => serialGridMeta.value.serials,
  () => {
    if (serialGridMeta.value.serials.length > 0) {
      const uniqueIDFound = serialGridMeta.value?.serials.find(
        (serial) => serial?.serial === props.selectedSerial
      );
      if (uniqueIDFound)
        onSerialSelect({
          UniqueID: uniqueIDFound.UniqueID,
          class: "bg-gray-200",
          serial: props.selectedSerial,
        });
    }
  }
);
watch(
  ...
  () => {
    if (complaintGridMeta.value.complaints.length > 0) {
      const uniqueIDFound = complaintGridMeta.value?.complaints.find(
        (complaint) => complaint?.COMPLAINTNUMBER === props.selectedComplaint
      );
...
<template>
  ...
  <UForm
    ...
    @submit="onSubmit"
  >
    <div class="w-full px-3 py-2 gmsPurpleTitlebar">Customer Information</div>

    <div class="!my-0 flex flex-row space-x-8 p-3">
      <div class="basis-1/2">
        <div class="flex justify-between">
          <div>Customer# {{ formData.number }}</div>
          <div>
        ...
        <div class="flex flex-row space-x-3">
          ...
          <div class="basis-1/2">
            ...
            <div class="flex flex-col mt-4 space-y-3">
              ...
              <div>
                {{ formData.billcity ? formData.billcity : "" }}
                {{ formData.billstate ? `, ${formData.billstate}` : "" }}
      ...
      <div class="basis-1/2">
          ...
          <div class="basis-1/6">
            <UFormGroup label="Select Serial" name="serial">
              <UTable
                ...
                :class="
                  formValidationErrors?.find((e) => e.path === 'serial')
              ...
              <div>
                <UFormGroup label="View Invoice" name="invoice">
                  <UTable
                    ...
                    :ui="{
                      wrapper:
                        'h-[100px] border border-gray-400 dark:border-gray-700 gms-ModalFormText',
                      divide: 'divide-gray-200 dark:divide-gray-800',
                      tr: {
                        active: 'hover:bg-gray-200 dark:hover:bg-gray-800/50',
                      },
        ...
        <div class="mt-2">
          <div class="pl-3 gmsPurpleTitlebar py-1">Service Reports</div>
            ...
            <div class="flex flex-row">
              <div class="px-2 w-full">
                <UTable
                  ...
                  :ui="{
                    wrapper:
                      'h-24 border border-gray-400 dark:border-gray-700 gms-ModalFormText',
                    divide: 'divide-gray-200 dark:divide-gray-800',
                    tr: {
                      active: 'hover:bg-gray-200 dark:hover:bg-gray-800/50',
                    },
    ...
    <div class="!my-0 flex flex-row">
      <div
        class="basis-1/2 border-[1px] border-slate-600 border-l-0 border-b-0 border-t-0"
      >
        <div class="w-full bg-slate-400 px-3 py-2 gmsPurpleTitlebar">
          Service Order
        </div>
        <div class="flex flex-row px-3 py-2">
          <div class="basis-5/12 leading-6">
            <div class="font-bold">
              {{
                serviceOrderInfo?.COMPLAINTNUMBER
                  ? `# ${serviceOrderInfo.COMPLAINTNUMBER}`
                  : ""
              }}
            </div>
            ...
            <div>
              {{
                serviceOrderInfo?.SERIALNO
                  ? `Serial ${serviceOrderInfo.SERIALNO}`
                  : ""
              }}
            </div>
          </div>
          ...
          <div class="basis-4/12">
                ...
                <UFormGroup name="complaintDate">
                  <UPopover :popper="{ placement: 'bottom-start' }">
                    <UButton
                      ...
                      :label="
                        serviceOrderInfo.COMPLAINTDATE &&
                        format(serviceOrderInfo.COMPLAINTDATE, 'MM/dd/yyyy')
                      "
                      variant="outline"
                      :class="
                        formValidationErrors?.find(
                          (e) => e.path === 'complaintDate'
                        )
                          ? 'ring-red-500'
                          : ''
                      "
                    ...
                    <template #panel="{ close }">
                      <CommonDatePicker
                        v-model="serviceOrderInfo.COMPLAINTDATE"
                        is-required
                        @close="close"
                      />
                    </template>
            ...
            <div class="flex flex-row mt-3">
              <div class="flex items-center w-[35px] font-medium">
                Created By
              </div>
              <div class="flex-1 px-4">
                <USelect
        ...
        <div class="flex px-3 space-x-3">
          <div class="basis-1/2">
            <UFormGroup label="Contacted by" required>
              <UInput />
            </UFormGroup>
          </div>
          <div class="basis-1/2">
        ...
        <div class="px-3 py-0">
          <UFormGroup label="Description of Issue" name="description" required>
        ...
        <div class="px-3 py-0 mt-1">
          <div class="flex flex-row border border-gray-500 rounded-md py-1">
            ...
            <UFormGroup
              name="injuryReportNo"
              class="basis-3/12 flex flex-row items-center"
            >
            ...
            <div class="basis-4/12 flex flex-row space-x-5 justify-center">
              <UButton
                label="VIEW#1"
                color="gms-purple"
                @click="onView1BtnClick"
              />
              <UButton
                label="VIEW#2"
                color="gms-purple"
                @click="onView2BtnClick"
              />
            </div>
          </div>
        </div>
        <div class="px-3 py-0 mt-1">
          <div
            class="flex flex-col border border-gray-500 rounded-md px-2 py-1"
          >
              ...
              <div class="basis-3/12">
                <UFormGroup label="Received" name="received">
                  <UPopover :popper="{ placement: 'bottom-start' }">
                    <UButton
                      icon="i-heroicons-calendar-days-20-solid"
                      :label="date && format(date, 'd MMM, yyy')"
                      variant="outline"
                      :ui="{
                        base: 'w-full',
                        truncate: 'flex justify-center w-full',
      ...
      <div class="basis-1/2">
        <div class="w-full gmsPurpleTitlebar px-3 py-2">Type of Services</div>
        <div class="w-full flex flex-col p-3 space-y-2">
          <div class="flex flex-row space-x-4 pb-2">
            ...
            <div class="w-1/3 flex flex-col space-y-3">
              ...
              <div>This Warranty Cost: $0.00</div>
              <div>Total Serial Warranty: $2984.30</div>
              <div>Ship Date: 4/1/2007</div>
            </div>
          ...
          <div class="flex flex-row space-x-2">
            <div class="w-full p-2 pt-0 border border-gray-500">
              <UFormGroup label="Investigation Required" name="investigation">
                <UTable
                  ...
                  :ui="{
                    wrapper:
                      'h-32 border-2 border-gray-400 dark:border-gray-700 gms-ModalFormText',
                    divide: 'divide-gray-200 dark:divide-gray-800',
                    tr: {
                      active: 'hover:bg-gray-200 dark:hover:bg-gray-800/50',
                    },
          ...
          <div
            v-if="
              serviceOrderInfo.OPENCASE === '1' && serviceOrderInfo.CLOSEDOUTBY
            "
  ...
  >
    <ServiceReportDetail
      :selected-complaint="complaintGridMeta.selectedComplaint?.uniqueID"
      :selected-service-report="selectedServiceReportID"
      @save="onServiceReportSave"
      @close="onServiceReportClose"
    />
  </UDashboardModal>
  ...
  >
    <MaterialsTransactionsInventoryTransactions :is-page="false" />
  </UDashboardModal>
  <!-- New Invoice Modal -->
  <UDashboardModal
  ...
  >
    <EngineeringInvestigationDetail
      :selected-investigation="
        investigationGridMeta.selectedInvestigation?.investigationID ?? null
      "
      @close="onInvestigationModalClose"
      @link="onInvestigationAdd"
    />
  </UDashboardModal>
  ...
  >
    <EngineeringNonconformanceDetail
      @link="onNonConformanceLink"
      @close="onNonConformanceModalClose"
    />
  </UDashboardModal>
  <!-- Injury Report1 Modal -->
  <UDashboardModal
    v-model="modalMeta.isInjuryReport1ModalOpen"
    title="Patient Injury Report #1"
    :ui="{
      title: 'text-lg text-white',
      header: {
  ...
  >
    <template #footer>
      <UButton
        ...
        variant="outline"
        color="red"
        :ui="{ base: 'w-24', truncate: 'flex justify-center w-full' }"
        truncate
        @click="warnMsg.warnMsgModalOpen = false"
      />
    </template>
  </UDashboardModal>
  ...
  >
    <template #footer>
      ...
      <UButton
        ...
        :ui="{ base: 'w-24', truncate: 'flex justify-center w-full' }"
        truncate
        @click="showOnSaveAlertModal = false"
      />
      <UButton
        label="Cancel"
        variant="outline"
