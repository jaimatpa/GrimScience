  ...
  await useApiFetch(`/api/tbl/tblEmployee?ACTIVE=1`, {
    ...
    onResponse({ response }) {
      if (response.status === 200) {
        employees.value = response._data.body.map(
...
if (props.selectedComplaint) editInit();
else propertiesInit();
</script>
              ...
              <UFormGroup label="Date Reported">
                <UPopover :popper="{ placement: 'bottom-start' }">
                  <UButton
                    ...
                    :label="
                      formData.DATEREPORTED2 &&
                      format(formData.DATEREPORTED2, 'dd/MM/yyyy')
                    "
                    variant="outline"
                    :ui="{ base: 'w-full' }"
                  />
                  <template #panel="{ close }">
                    <CommonDatePicker
                      v-model="formData.DATEREPORTED2"
                      is-required
                      @close="close"
                    />
                  </template>
              ...
              <UFormGroup label="Date of Injury">
                <UPopover :popper="{ placement: 'bottom-start' }">
                  <UButton
                    icon="i-heroicons-calendar-days-20-solid"
                    :label="
                      formData.DATEINJURED2 &&
                      format(formData.DATEINJURED2, 'dd/MM/yyyy')
                    "
                    variant="outline"
                    :ui="{ base: 'w-full' }"
                  />
                    ...
                    <UFormGroup label="Date of Last Visit">
                      <UPopover :popper="{ placement: 'bottom-start' }">
                        <UButton
                          ...
                          :label="
                            formData.LASTVISIT2 &&
                            format(formData.LASTVISIT2, 'dd/MM/yyyy')
                          "
                          variant="outline"
                          :ui="{ base: 'w-[250px]' }"
                        />
                ...
                <div class="p-2 w-full">
                  ...
                  <div class="ml-[68px] mt-[-28px]">
                    <UPopover
                      ...
                      :ui="{ trigger: 'ml-[0px] mt-[0px]' }"
                    >
                      <UButton
                        icon="i-heroicons-calendar-days-20-solid"
                        :label="
                          formData.DEATH2 &&
                          format(formData.DEATH2, 'dd/MM/yyyy')
                        "
                        variant="outline"
                        :ui="{ base: 'w-[250px]' }"
                      />
          ...
          <UFormGroup label="IMPORTANT!">
            <div class="p-2">
              <p class="font-semibold">
                Device related death, serious injury, or malfunctions must be
                reported to FDA within 30 calendar days from becoming aware of
                the event. Notify Grimm Scientifics' President within 1 hour of
                any patient injury.
              </p>
                      ...
                      <div class="basis-1/2">
                        <UPopover :popper="{ placement: 'bottom-start' }">
                          <UButton
                            icon="i-heroicons-calendar-days-20-solid"
                            :label="
                              formData.REPORTBYDATE2 &&
                              format(formData.REPORTBYDATE2, 'dd/MM/yyyy')
                            "
                            variant="outline"
                            :ui="{ base: 'w-full' }"
                          />
                          <template #panel="{ close }">
                            <CommonDatePicker
                              v-model="formData.REPORTBYDATE2"
                              is-required
                              @close="close"
                            />
                          </template>
                      ...
                      <div class="basis-1/2">
                        <UPopover :popper="{ placement: 'bottom-start' }">
                          <UButton
                            icon="i-heroicons-calendar-days-20-solid"
                            :label="
                              formData.REVIEWBYDATE2 &&
                              format(formData.REVIEWBYDATE2, 'dd/MM/yyyy')
                            "
                            variant="outline"
                            :ui="{ base: 'w-full' }"
                          />
                          <template #panel="{ close }">
                            <CommonDatePicker
                              v-model="formData.REVIEWBYDATE2"
                              is-required
                              @close="close"
                            />
                          </template>
