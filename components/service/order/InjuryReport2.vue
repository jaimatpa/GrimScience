...
<template>
  ...
  <UForm :validate="validate" :validate-on="['submit']" :state="formData">
    <div class="flex flex-col">
          ...
          <div class="basis-7/12 flex flex-row space-x-2">
              ...
              <UFormGroup label="Date Reported">
                <UPopover :popper="{ placement: 'bottom-start' }">
                  <UButton icon="i-heroicons-calendar-days-20-solid"
                    :label="formData.DATEREPORTED2 && format(formData.DATEREPORTED2, 'dd/MM/yyyy')" variant="outline"
                    :ui="{ base: 'w-full' }" />
            ...
            <div class="basis-1/3">
              <UFormGroup label="Date of Injury">
                <UPopover :popper="{ placement: 'bottom-start' }">
                  <UButton icon="i-heroicons-calendar-days-20-solid"
                    :label="formData.DATEINJURED2 && format(formData.DATEINJURED2, 'dd/MM/yyyy')" variant="outline"
                    :ui="{ base: 'w-full' }" />
                  <template #panel="{ close }">
      ...
      <div
        class="px-4 pt-2 border-[1px] border-slate-600 border-l-0 border-b-0 border-r-0 mt-1 flex flex-col space-y-2">
              ...
              <UFormGroup label="1. Was patient treated at hospital?">
                <div class="p-2">
                  <div class="flex flex-row space-x-5">
                    <URadio v-for="status of statusGroup" :key='status.value' v-model="formData.PATIENTTREATED2"
                      v-bind="status" />
                  </div>
                  <div class="mt-1">
            ...
            <div class="border-[1px] border-slate-300 p-3 pt-1 pb-0">
              <UFormGroup label="2. Did physician trean injured patient?">
                <div class="p-2 flex flex-row space-x-3">
                  <div class="basis-1/2">
                    <div class="flex flex-row space-x-5">
                      <URadio v-for="status of statusGroup" :key='status.value' v-model="formData.PHYSTREAT2"
                        v-bind="status" />
                    </div>
                    <div class="mt-1">
          ...
          <div class="basis-5/12">
            <div class="border-[1px] border-slate-300 p-3 pt-1 pb-0">
              <UFormGroup label="3. Is patient still under physician's care?">
                <div class="p-2">
                  <div class="flex flex-row space-x-5">
                    <URadio v-for="status of statusGroup" :key='status.value' v-model="formData.UNDERCARE2"
                      v-bind="status" />
                  </div>
                  <div class="mt-1">
                    <UFormGroup label="Date of Last Visit">
                      <UPopover :popper="{ placement: 'bottom-start' }">
                        <UButton icon="i-heroicons-calendar-days-20-solid"
                          :label="formData.LASTVISIT2 && format(formData.LASTVISIT2, 'dd/MM/yyyy')" variant="outline"
                          :ui="{ base: 'w-[250px]' }" />
                        <template #panel="{ close }">
              ...
              <UFormGroup label="4. Indicate Nature of Injury">
                <div class="p-2 flex flex-row">
                  <div class="basis-1/2 space-y-2">
                    <URadio v-for="injury of injuryGroup" :key='injury.value' v-model="formData.INJURYNATURE2"
                      v-bind="injury" />
                  </div>
                  <div class="flex items-end">
                    <UPopover :popper="{ placement: 'bottom-start' }" :ui="{ trigger: 'ml-[-370px] pt-[50px]' }">
                      <UButton icon="i-heroicons-calendar-days-20-solid"
                        :label="formData.DEATH2 && format(formData.DEATH2, 'dd/MM/yyyy')" variant="outline"
                        :ui="{ base: 'w-[250px]' }" />
                      <template #panel="{ close }">
        ...
        <div class="border-[1px] border-slate-300 p-3 pb-0">
          <UFormGroup label="IMPORTANT!">
            <div class="p-2">
              <p class="font-semibold">Device related death, serious injury, or malfunctions must be reported to FDA
                within 30
                calendar days from becoming aware of the event. Notify Grimm Scientifics' President within 1 hour of any
                patient injury.</p>
                ...
                <UFormGroup label="Received By">
                  <div class="flex flex-row space-x-4">
                    ...
                    <div class="min-w-[350px]">
                      <UPopover :popper="{ placement: 'bottom-start' }">
                        <UButton icon="i-heroicons-calendar-days-20-solid"
                          :label="formData.REPORTBYDATE2 && format(formData.REPORTBYDATE2, 'dd/MM/yyyy')"
                ...
                <UFormGroup label="Reviewed By">
                  <div class="flex flex-row space-x-4">
                    ...
                    <div class="min-w-[350px]">
                      <UPopover :popper="{ placement: 'bottom-start' }">
                        <UButton icon="i-heroicons-calendar-days-20-solid"
                          :label="formData.REVIEWBYDATE2 && format(formData.REVIEWBYDATE2, 'dd/MM/yyyy')"
                          variant="outline" :ui="{ base: 'w-full' }" />
                        <template #panel="{ close }">
      ...
      <div class="mt-2 px-4 flex flex-row-reverse">
        <div>
          <UButton icon="i-f7-rays" label="Clear From" color="red" variant="outline"
            :ui="{ base: 'min-w-[200px]', truncate: 'flex justify-center w-full' }" truncate />
        </div>
        <div class="mr-4">
          <UButton icon="i-heroicons-document-text-20-solid" label="Save" color="green" variant="outline"
