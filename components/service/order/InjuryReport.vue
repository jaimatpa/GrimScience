          ...
          <div class="basis-7/12 flex flex-row space-x-2">
              ...
              <UFormGroup label="Date Reported">
                <UPopover :popper="{ placement: 'bottom-start' }">
                  <UButton icon="i-heroicons-calendar-days-20-solid"
                    :label="formData.DATEREPORTED && format(formData.DATEREPORTED, 'dd/MM/yyyy')" variant="outline"
                    :ui="{ base: 'w-full' }" />
            ...
            <div class="basis-1/3">
              <UFormGroup label="Date of Injury">
                <UPopover :popper="{ placement: 'bottom-start' }">
                  <UButton icon="i-heroicons-calendar-days-20-solid"
                    :label="formData.DATEINJURED && format(formData.DATEINJURED, 'dd/MM/yyyy')" variant="outline"
                    :ui="{ base: 'w-full' }" />
                  <template #panel="{ close }">
              ...
              <UFormGroup label="1. Was patient treated at hospital?">
                <div class="p-2">
                  <div class="flex flex-row space-x-5">
                    <URadio v-for="status of statusGroup" :key='status.value' v-model="formData.PATIENTTREATED"
                      v-bind="status" />
                  </div>
                  <div class="mt-1">
            ...
            <div class="border-[1px] border-slate-300 p-3 pt-1 pb-0">
              <UFormGroup label="2. Did physician trean injured patient?">
                <div class="p-2 flex flex-row space-x-3">
                  <div class="basis-1/2">
                    <div class="flex flex-row space-x-5">
                      <URadio v-for="status of statusGroup" :key='status.value' v-model="formData.PHYSTREAT"
                        v-bind="status" />
                    </div>
                    <div class="mt-1">
          ...
          <div class="basis-5/12">
            <div class="border-[1px] border-slate-300 p-3 pt-1 pb-0">
              <UFormGroup label="3. Is patient still under physician's care?">
                <div class="p-2">
                  <div class="flex flex-row space-x-5">
                    <URadio v-for="status of statusGroup" :key='status.value' v-model="formData.UNDERCARE"
                      v-bind="status" />
                  </div>
                  <div class="mt-1">
                    <UFormGroup label="Date of Last Visit">
                      <UPopover :popper="{ placement: 'bottom-start' }">
                        <UButton icon="i-heroicons-calendar-days-20-solid"
                          :label="formData.LASTVISIT && format(formData.LASTVISIT, 'dd/MM/yyyy')" variant="outline"
                          :ui="{ base: 'w-[250px]' }" />
                        <template #panel="{ close }">
              ...
              <UFormGroup label="4. Indicate Nature of Injury">
                <div class="p-2 flex flex-row">
                  <div class="basis-1/2 space-y-2">
                    <URadio v-for="injury of injuryGroup" :key='injury.value' v-model="formData.INJURYNATURE"
                      v-bind="injury" />
                  </div>
                  <div class="flex items-end">
                    <UPopover :popper="{ placement: 'bottom-start' }" :ui="{ trigger: 'ml-[-370px] pt-[50px]' }">
                      <UButton icon="i-heroicons-calendar-days-20-solid"
                        :label="formData.DEATH && format(formData.DEATH, 'dd/MM/yyyy')" variant="outline"
                        :ui="{ base: 'w-[250px]' }" />
                      <template #panel="{ close }">
                ...
                <UFormGroup label="Received By">
                  <div class="flex flex-row space-x-4">
                    ...
                    <div class="min-w-[350px]">
                      <UPopover :popper="{ placement: 'bottom-start' }">
                        <UButton icon="i-heroicons-calendar-days-20-solid"
                          :label="formData.REPORTBYDATE && format(formData.REPORTBYDATE, 'dd/MM/yyyy')"
                ...
                <UFormGroup label="Reviewed By">
                  <div class="flex flex-row space-x-4">
                    ...
                    <div class="min-w-[350px]">
                      <UPopover :popper="{ placement: 'bottom-start' }">
                        <UButton icon="i-heroicons-calendar-days-20-solid"
                          :label="formData.REVIEWBYDATE && format(formData.REVIEWBYDATE, 'dd/MM/yyyy')"
                          variant="outline" :ui="{ base: 'w-full' }" />
                        <template #panel="{ close }">
