...
<template>
            ...
            <div class="flex flex-row space-x-5">
              ...
              <div class="basis-1/2">
                <div class="flex justify-between">
                  ...
                  <USelect v-model="formData.REPAIRSBY" :options="serviceTechOptions" />
                </div>
              </div>
            </div>
            <div class="flex flex-row space-x-5">
      ...
      <div class="basis-1/3">
        ...
        <div class="p-3">
          ...
          <div class="flex flex-row space-x-2 mt-2 fheight-btn-row">
            ...
            <div class="basis-1/3 w-full">
              <UButton icon="i-heroicons-plus-20-solid" label="Create Invoice" variant="outline" color="green"
                :ui="{ base: 'w-full', truncate: 'flex justify-center break-words text-center w-full' }" truncate />
            </div>
            <div class="basis-1/3 w-full">
      ...
      <div class="flex flex-row space-x-3 p-3">
        <div class="w-1/2">
          <UTable :rows="warrantyMaterialGridMeta.warrantyMaterials" :columns="warrantyMaterialGridMeta.defaultColumns"
            ...
            @select="onWarrantyMaterialSelect" @dblclick="onWarrantyMaterialDblClick">
            <template v-for="column in warrantyMaterialGridMeta.defaultColumns" v-slot:[`${column.key}-header`]>
              <template v-if="!column.filterOptions">
                <div class="px-1 py-1">
                  <CommonSortAndInputFilter @handle-sorting-button=""
                    @handle-input-change="onWarrantyMaterialFilterChange" :label="column.label"
              ...
              <template v-else>
                <div class="px-1 py-1 min-w-[120px]">
                  <CommonSortAndSelectFilter @handle-sorting-button=""
                    @handle-select-change="onWarrantyMaterialFilterChange" :label="column.label"
                    :sortable="column.sortable" :sort-key="column.key"
                    :sort-icon="column?.sortDirection === 'none' ? noneIcon : column?.sortDirection === 'asc' ? ascIcon : descIcon"
                    :value="warrantyMaterialFilterValues[column.key]" :filterable="column.filterable"
        ...
        <div class="w-1/2">
          ...
          <UTable :rows="selectedWarrantyMaterialGridMeta.warrantyMaterials"
            ...
            @select="onSelectedWarrantySelect" @dblclick="onSelectedWarrantyDblclick">
            <template #empty-state>
              <div></div>
            </template>
          </UTable>
      ...
      <div class="flex flex-row space-x-3 p-3">
        <div class="w-1/2">
          <UTable :rows="partGridMeta.parts" :columns="partGridMeta.defaultColumns" :loading="partGridMeta.isLoading"
            ...
            }" :empty-state="{ icon: 'i-heroicons-circle-stack-20-solid', label: 'No items.' }" @select="onPartSelect"
            @dblclick="onPartDblClick">
            <template v-for="column in partGridMeta.defaultColumns" v-slot:[`${column.key}-header`]>
              <template v-if="!column.filterOptions">
                <div class="px-1 py-1">
                  <CommonSortAndInputFilter @handle-sorting-button="" @handle-input-change="onPartFilterChange"
              ...
              <template v-else>
                <div class="px-1 py-1 min-w-[120px]">
                  <CommonSortAndSelectFilter @handle-sorting-button="" @handle-select-change="onPartFilterChange"
                    :label="column.label" :sortable="column.sortable" :sort-key="column.key"
                    :sort-icon="column?.sortDirection === 'none' ? noneIcon : column?.sortDirection === 'asc' ? ascIcon : descIcon"
                    :value="partFilterValues[column.key]" :filterable="column.filterable" :filter-key="column.key"
        ...
        <div class="w-1/2">
          <UTable :rows="selectedPartGridMeta.parts" :columns="selectedPartGridMeta.defaultColumns"
            ...
            }" @select="onSelectedPartSelect" @dblclick="onSelectedPartDblclick">
            <template #empty-state>
              <div></div>
            </template>
          </UTable>
  ...
  <UDashboardModal v-model="addModalMeta.isAddModalOpen" :ui="{
    ...
    <div>
      <div class="flex flex-row space-x-5">
        ...
        <div class="flex-1 mr-4">
          <UInput type="number" :ui="{ base: qtyStyle }" v-model="addModalMeta.quantity"
            @keypress="(event) => { if (event.which === 13) onAddWarrantyOrMaterial() }"></UInput>
        </div>
      </div>
      <div class="flex flex-row-reverse mt-2">
        <div class="min-w-[60px]">
          <UButton label="OK" :ui="{ base: 'w-full', truncate: 'flex justify-center w-full' }" type="submit" truncate
            @click="onAddWarrantyOrMaterial" />
        </div>
        <div class="min-w-[60px] mr-3">
          <UButton label="Cancel" :ui="{ base: 'w-full', truncate: 'flex justify-center w-full' }" truncate
            @click="addModalMeta.isAddModalOpen = false" />
        </div>
      </div>
    </div>
  </UDashboardModal>
