...
const editInit = async () => {
    ...
    onResponse({ response }) {
      if (response.status === 200) {
        for (const key in response._data.body) {
          if (
            response._data.body[key] !== undefined &&
  ...
  await useApiFetch(`/api/tbl/tblSourceCodes?source=${formData.source}`, {
    ...
    onResponse({ response }) {
      if (response.status === 200) {
        sourcedescriptionOptions.value = response._data.body.map(
...
const propertiesInit = async () => {
  ...
  await useApiFetch(
    `/api/customers/${
      props.selectedCustomer ? props.selectedCustomer : formData.customerid
    }`,
    {
      method: "GET",
      onResponse({ response }) {
    ...
    onResponse({ response }) {
      if (response.status === 200) {
        if (response._data?.body?.length) {
          ...
          sourceOptions.value = sourceCodes.value
            .map((item) => item.source)
            .sort();
          sourceOptions.value = sourceOptions.value.filter(
            (item, index) =>
              sourceOptions.value.indexOf(item) === index &&
              item != "" &&
              item != null
          );
  ...
  await useApiFetch(`/api/tbl/tblEmployee?ACTIVE=1`, {
    ...
    onResponse({ response }) {
      if (response.status === 200) {
        if (response._data?.body?.length) {
          soldByOptions.value = response._data.body.map(
            (item) =>
              `#${item.payrollnumber || "n/a"} ${item.fname || ""} ${
                item.lname || ""
              }`
          );
          soldByOptions.value.unshift(null);
          installationByOptions.value = response._data.body.filter((item) =>
            [1, 65, 62, 18, 44].includes(item.UniqueID)
          );
          installationByOptions.value = installationByOptions.value
            .map(
              (item) =>
                `#${item.payrollnumber || "n/a"} ${item.fname || ""} ${
                  item.lname || ""
...
const fetchProducts = async () => {
  try {
    await useApiFetch("/api/invoices/productLine", {
      method: "GET",
      params: {
        ...orderMeta,
      },
...
const onSourceItemChange = async (optionStr) => {
  sourcedescriptionOptions.value = sourceCodes.value
    .filter(
      (item) =>
        item.source === optionStr && item.source != "" && item.source != null
    )
...
const onProductSelect = (row) => {
  if (
    JSON.stringify({ ...selectedProduct.value, class: "" }) ===
...
const onOrderSelect = (row) => {
  ...
  orderList.value.forEach((order) => {
    if (
      order.UniqueID === row.UniqueID &&
      order.created === row.created &&
      row.class != "bg-gray-200"
    ) {
...
const handleAddBtnClick = () => {
  ...
  if (selectedProduct.value && qty.value) {
    const newOrder = {
      ...
      type: selectedProduct.value.model,
...
const handleRemoveBtnClick = async () => {
  if (selectedOrder.value) {
    const index = orderList.value.findIndex(
      (order) =>
        order.UniqueID === selectedOrder.value.UniqueID &&
        order.created === selectedOrder.value.created
    );
    if (index > -1) {
      orderList.value.splice(index, 1);
      if (selectedOrder?.value?.UniqueID ?? null !== null) {
        await useApiFetch(
          `/api/invoices/detail/${selectedOrder.value.UniqueID}`,
          {
            method: "DELETE",
            onResponse() {},
            onResponseError() {},
...
const onUpdatePrice = () => {
  ...
  const index = orderList.value.findIndex(
    (item) =>
      item.UniqueID === selectedOrder.value.UniqueID &&
      item.created === selectedOrder.value.created
  );
  orderList.value.splice(index, 1, {
    ...orderList.value[index],
    PRIMARYPRICE1: updatedPrice.value,
  });
  onCalculateInvoiceValues();
...
const onCalculateInvoiceValues = () => {
  ...
  orderList.value.forEach((order) => {
    itemsTotal.value +=
      (order?.quantity ?? (0 as number)) *
      (order?.PRIMARYPRICE1 ?? (0 as number));
  });

  formData.subtotal =
    Number(itemsTotal.value) -
    Number(formData.lessdiscount) -
    Number(formData.lessdown) +
    Number(formData.shipping);
  formData.total =
    Number(formData.subtotal) + (mdetChecked.value ? Number(formData.MDET) : 0);
...
const onProcessCreditCard = async () => {
  ...
  if (
    creditCardMeta.value.expirationmonth < 1 ||
    creditCardMeta.value.expirationmonth > 13
  ) {
      ...
      onResponse({ response }) {
        if (response._data.messages.resultCode === "Ok") {
          toast.add({
            title: "Success",
            description:
              response._data?.transactionResponse?.messages[0]?.description ??
              "",
          ...
          toast.add({
            title: "Fail",
            description:
              response._data?.transactionResponse?.errors[0]?.errorText ??
              response._data?.messages?.message[0]?.text,
    ...
    <div class="flex flex-row">
      <div class="basis-4/6 border-r-[3px] border-black">
        <div class="!my-0 flex flex-row">
          <div class="basis-2/5">
            ...
            <div class="flex flex-col p-3 space-y-2">
                  ...
                  <UFormGroup label="Quote Date(Invoice Date)" name="quoteDate">
                    <UPopover :popper="{ placement: 'bottom-start' }">
                      <UButton
                        ...
                        :label="
                          formData.orderdate &&
                          format(formData.orderdate, 'MM/dd/yyyy')
                        "
                        variant="outline"
                        :ui="{
                          base: 'w-full',
              ...
              <div class="flex flex-row items-center">
                <div class="font-medium min-w-[137px]">Source</div>
              ...
              <div class="flex flex-row items-center">
                <div class="font-medium min-w-[137px]">Source Description</div>
                <div class="flex-1 pl-4">
                  <USelect
              ...
              <div class="flex flex-row items-center">
                <div class="font-medium min-w-[137px]">PO#</div>
                <div class="flex-1 pl-4">
                  <UInput v-model="formData.purchaseordernumber" />
                </div>
              </div>
              <div class="flex flex-row items-center">
                <div class="font-medium min-w-[137px]">Sold By</div>
                <div class="flex-1 pl-4">
                  <USelect v-model="formData.soldby" :options="soldByOptions" />
                </div>
              </div>
              <div class="flex flex-row items-center">
                <div class="font-medium min-w-[137px]">Quote#</div>
                <div class="flex-1 pl-4">
                  <UInputMenu
          ...
          <div
            class="basis-3/5 border-[1px] border-slate-600 border-b-0 border-r-0 border-t-0"
          >
            <div class="w-full px-3 py-1 gmsPurpleTitlebar">
              Customer Informaton
            </div>
              ...
              <div class="flex flex-row space-x-3">
                <div class="basis-1/2">
                  ...
                  <div class="flex flex-col mt-4 space-y-3">
                    <div>
                      {{ customerData.fname ? customerData.fname : "" }}
                      {{ customerData.lname ? customerData.lname : "" }}
                    </div>
                    <div>
                ...
                <div class="basis-1/2">
                  ...
                  <div class="flex flex-col mt-4 space-y-3">
                    ...
                    <div>
                      ...
                      {{
                        customerData.billstate
                          ? `,
                      ${customerData.billstate}`
                          : ""
                      }}
                      {{
                        customerData.billzip ? `, ${customerData.billzip}` : ""
                      }}
                    </div>
                    <div>
                      {{
                        customerData.billphone
                          ? `P: ${customerData.billphone}`
                          : ""
                      }}
                    </div>
                  </div>
        ...
        <div class="w-full px-3 py-1 gmsPurpleTitlebar">Items Ordered</div>
        <div class="w-full p-3">
          <div class="flex flex-col space-y-2">
            <div class="flex flex-row justify-end">
              <div class="flex space-x-3">
                <div>Show Only Available</div>
                <div>
                  <UCheckbox
                    name="fix"
                    v-model="orderMeta.showOnlyInventory"
            ...
            <div>
              <UTable
                ...
                :ui="{
                  wrapper:
                    'h-32 border-[1px] border-gray-400 dark:border-gray-700',
                  tr: {
            ...
            <div class="flex flex-row justify-between space-x-4">
              <div
                class="basis-2/3 border border-gray-400 dark:border-gray-700 py-1 px-3"
              >
                <UFormGroup
      ...
      <div class="basis-2/6 w-full">
        <div class="w-full px-3 py-1 gmsPurpleTitlebar">Order Information</div>
                  ...
                  <UPopover :popper="{ placement: 'bottom-start' }">
                    <UButton
                      ...
                      :label="
                        ...
                        format(formData.invoicedate, 'MM/dd/yyyy')
                      "
                      variant="outline"
                      :ui="{
                        base: 'w-full',
                  ...
                  <UPopover :popper="{ placement: 'bottom-start' }">
                    <UButton
                      ...
                      :label="
                        ...
                        format(formData.acceptancedate, 'MM/dd/yyyy')
                      "
                      variant="outline"
                      :ui="{
                        base: 'w-full',
                ...
                <UFormGroup label="Date Shipped" name="shippedDate">
                  <UPopover :popper="{ placement: 'bottom-start' }">
                    <UButton
                      ...
                      :label="
                        formData.shipdate &&
                        format(formData.shipdate, 'MM/dd/yyyy')
                      "
                      variant="outline"
                      :ui="{
                        base: 'w-full',
            ...
            <div class="flex flex-col space-y-2">
              <div class="flex flex-row space-x-2">
                <div class="basis-1/2 flex items-end">
                  ...
                  <div
                    class="flex-1 ms-1"
                    style="border-bottom: 2px dotted black"
                  ></div>
                </div>
                <div class="basis-1/2">
                  <UInput
              ...
              <div class="flex flex-row space-x-2">
                <div class="basis-1/2 flex items-end">
                  <div class="flex flex-row space-x-1 w-full">
                    <div class="flex-auto">Sales Tax</div>
                    <div class="flex-auto">
              ...
              <div class="flex flex-row space-x-2">
                <div class="basis-1/2 flex items-end">
                  <div class="flex flex-row space-x-1 w-full">
                    <div class="flex-auto">MDET 2.3 %</div>
                    <div class="flex-auto">
                      <div class="flex flex-row items-center space-x-1">
                        <div>
                          <UCheckbox
                            name="mdet"
                            v-model:model-value="mdetChecked"
                            @change="onCalculateInvoiceValues"
                          />
                        </div>
                        <div>(Apply)</div>
            ...
            <div class="flex flex-row space-x-2 w-full items-center">
              <div class="basis-1/2 w-full flex justify-center">
                <UButton
                  ...
                  color="gms-purple"
                  :ui="{ base: 'justify-center w-full' }"
                  @click="onProcessCreditCardBtnClick"
                />
              </div>
              <div class="basis-1/2">
                <div class="flex justify-between items-center">
                  <div class="basis-1/2">Last 4 Digits</div>
                  <div class="basis-1/2">
                    <UInput
        ...
        <div class="w-full px-3 py-1 gmsPurpleTitlebar">Printing Options</div>
            ...
            <div class="w-[100px]">
              <UInput
                ...
                @change="
                  () => {
                    if (
                      creditCardMeta.expirationmonth < 1 ||
                      creditCardMeta.expirationmonth > 13
                    ) {
